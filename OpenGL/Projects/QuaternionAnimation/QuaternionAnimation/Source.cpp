...
void intToString(char * destStr, int val)
{
	...
	std::string dest = std::to_string(val);		

	for (int i = 0; i < dest.length(); i++)
		destStr[i] = dest[i];
	// char *cstring = new char[dest.length() + 1];
	 //strcpy(cstring, dest.c_str());
...
int main(int argc, char **argv)
{
	...
	glutCreateWindow("Quaternion Animation");
