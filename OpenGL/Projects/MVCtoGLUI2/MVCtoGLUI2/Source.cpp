...
void setCamera(float posX, float posY, float posZ, float targetX, float targetY, float targetZ)
{
	...
	matrixView[0] = glm::vec4(left[0], left[1], left[2], left[3]);
	matrixView[1] = glm::vec4(up[0], up[1], up[2], up[3]);
	matrixView[2] = glm::vec4(forward[0], forward[1], forward[2], forward[3]);
	matrixView[3] = glm::vec4(position[0], position[1], position[2], position[3]);
...
void drawSub1()
{
	...
	// clear buffer
	setViewportSub(0, halfHeight, halfHeight, halfHeight, 1, 10);
...
*/  /* glTranslatef(-cameraPosition[0], -cameraPosition[1], -cameraPosition[2]);
     glRotatef(cameraModelAngleX, 1, 0, 0);
      glRotatef(cameraModelAngleY, 0, 1, 0);*/
 // glPopMatrix();

  //setViewMatrix((float&)getCameraX, (float&)getCameraY, (float&)getCameraZ,(float&)getCameraAngleX, (float&)getCameraAngleY, (float&)getCameraAngleZ);
   glm::mat4 TransV;
   for (int i = 0; i < 4; i++) {
	   for (int j = 0; j < 4; j++) {
		   TransV[i][j] = matrixView[j][i];
		   //printf("%f,%f", Inv[i][j], Matrix[j][i]);
	   }
   }
   glLoadMatrixf(glm::value_ptr(TransV));
 //  setCameraAngleX(cameraModelAngleX);
  // setCameraAngleY(cameraModelAngleY);
   //glRotatef(cameraAngleX, 1, 0, 0);
   //glRotatef(cameraAngleY, 0, 1, 0);
  // setViewMatrix(cameraPosition[0], cameraPosition[1], cameraPosition[2], cameraModelAngleX, cameraModelAngleY, cameraAngle[2]);
  // setCameraX(-cameraPosition[0]);
  // setCameraY(cameraAngleX);
  // glPushMatrix();
  ...
  glm::mat4 TransMV;
   for (int i = 0; i < 4; i++) {
	   for (int j = 0; j < 4; j++) {
		   TransMV[i][j] = matrixModelView[j][i];
		   //printf("%f,%f", Inv[i][j], Matrix[j][i]);
	   }
   }

	glLoadMatrixf(glm::value_ptr(transpose(TransMV)));
...
//-------
	spinnerCamera[0] = new GLUI_Spinner(glui, "X", &spcameraX, 6, control_cb);
	spinnerCamera[0]->set_int_limits(-10, 10);
	spinnerCamera[1] = new GLUI_Spinner(glui, "Y", &spcameraY, 7, control_cb);
	spinnerCamera[1]->set_int_limits(-10, 10);
	spinnerCamera[2] = new GLUI_Spinner(glui, "Z", &spcameraZ, 8, control_cb);
	spinnerCamera[2]->set_int_limits(-10, 10);
	spinnerCamera[3] = new GLUI_Spinner(glui, "Pitch", &spcameraPitch, 9, control_cb);
	spinnerCamera[3]->set_int_limits(-10, 10);
	spinnerCamera[4] = new GLUI_Spinner(glui, "Heading", &spcameraHeading, 10, control_cb);
	spinnerCamera[4]->set_int_limits(-10, 10);
	spinnerCamera[5] = new GLUI_Spinner(glui, "Roll", &spcameraRoll, 11, control_cb);
	spinnerCamera[5]->set_int_limits(-10, 10);

//-------
	spinnerModel[0] = new GLUI_Spinner(glui, "X", &spmodelX, 12, control_cb);
	spinnerModel[0]->set_int_limits(-10, 10);
	spinnerModel[1] = new GLUI_Spinner(glui, "Y", &spmodelY, 13, control_cb);
	spinnerModel[1]->set_int_limits(-10, 10);
	spinnerModel[2] = new GLUI_Spinner(glui, "Z", &spmodelZ, 14, control_cb);
	spinnerModel[2]->set_int_limits(-10, 10);
	spinnerModel[3] = new GLUI_Spinner(glui, "Pitch", &spmodelPitch, 15, control_cb);
	spinnerModel[3]->set_int_limits(-10, 10);
	spinnerModel[4] = new GLUI_Spinner(glui, "Heading", &spmodelHeading, 16, control_cb);
	spinnerModel[4]->set_int_limits(-10, 10);
	spinnerModel[5] = new GLUI_Spinner(glui, "Roll", &spmodelRoll, 17, control_cb);
